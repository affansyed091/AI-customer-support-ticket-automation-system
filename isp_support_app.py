...
# ══════════════════════════════════════════════
#  GLOBAL CSS  (completely redesigned - but keeping all original classes)
# ══════════════════════════════════════════════
st.markdown("""
<style>
...
/* Unified chat container (quick topics + conversation) */
...
/* ── all your existing classes below (cust-card, form-box, topbar, etc.) remain unchanged ── */
/* Keep the rest of your original CSS exactly as you had it */
</style>
""", unsafe_allow_html=True)

# ══════════════════════════════════════════════
#  ALL YOUR ORIGINAL CONSTANTS, DB, AI, HELPERS GO HERE
#  (I'm keeping them exactly as you provided - no changes)
# ══════════════════════════════════════════════
...
# ══════════════════════════════════════════════
#  CHAT RENDERER (modified to show names)
# ══════════════════════════════════════════════
        ...
        if msg["role"] == "user":
            ...
            <div class="row user-row">
              <div class="umsg">
                ...
                <div class="utime">{htmllib.escape(ts)} · {htmllib.escape(customer_name)}</div>
              </div>
            ...
            <div class="row bot-row">
              ...
              <div class="acard">
                <div class="ahdr">
                  <span class="albl">✦ FiberISP AI Assistant</span>
...
defaults = {
    ...
    "active_tab":     0,   # for custom tab switching
}
...
if st.session_state.screen == "welcome":
    ...
    # CSS to fuse cards with buttons
    st.markdown("""
    <style>
...
# ══════════════════════════════════════════════
#  SCREEN: CUSTOMER DASHBOARD (modified)
# ══════════════════════════════════════════════
elif st.session_state.screen == "customer_dashboard":
    cust  = st.session_state.customer
    phone = st.session_state.phone

    # Customer card with outage warning
    outage_html = ""
    ...
    # Custom Tabs (to allow switching from floating logo)
    tab_names = ["💬  AI Support Chat", "🔌  New Connection", "💳  My Bill & Tickets", "⬆️  Upgrade Plan"]
    ...
    for i, name in enumerate(tab_names):
        ...
        if st.session_state.active_tab == i:
            ...
            div[data-testid="column"]:nth-child({i+1}) button {{
                background: linear-gradient(135deg, #0f2b3f, #142c44) !important;
                color: #38bdf8 !important;
    ...
    # ────────────────────────────────────────
    #  TAB 0: AI SUPPORT CHAT (unified container)
    # ────────────────────────────────────────
    ...
    # ────────────────────────────────────────
    #  TAB 1: NEW CONNECTION (unchanged from original)
    # ────────────────────────────────────────
    ...
    # ══════════════════════════════════════════════
    st.markdown("<div class='bottom-logout' style='margin-top:40px; text-align:center; border-top:1px solid rgba(14,165,233,0.12); padding-top:20px;'>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════
#  SCREEN: ADMIN DASHBOARD (unchanged, keep original)
# ══════════════════════════════════════════════
